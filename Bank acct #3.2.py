...
#create window to display transaction history
    ...
    def transfromSavings(self):
        ...
        if(float(self.transAmount.get()) > 0):
            ...
            TRANSACTION_HISTORY.append("Account: Savings || Type:  Transfer ||" + " Amount: " + self.transAmount.get() +
                                       " || Checking Balance: " + str(format(CHECKING_BALANCE, ".2f")) + " Savings Balance: " + str(format(SAVINGS_BALANCE, ".2f")))
            tkinter.messagebox.showinfo("Transfered", "Transfer successful! " + "savings $" + str(format(SAVINGS_BALANCE, ".2f")) +
                    " checking $" + str(format(CHECKING_BALANCE, ".2f")))
        self.window.destroy()
...
################################
#create savings account windows#
################################
...
class balanceSavings:
    ...
    def viewSavings(self,value):
        if(value=="yes"):
                global SAVINGS_BALANCE
                tkinter.messagebox.showinfo("Savings", "Your balance in " + "savings is $" + str(format(SAVINGS_BALANCE, ".2f")))
        self.window.destroy()
...
class SavingsPage:
    ...
    def addToSavings(self):
        ...
        if(float(self.depositAmount.get()) > 0):
            ...
            tkinter.messagebox.showinfo("Deposit", "Deposit successful! " + "savings $" + str(format(SAVINGS_BALANCE, ".2f")) +
                    " checking $" + str(format(CHECKING_BALANCE, ".2f")))
        self.window.destroy()

# withdraw from savings account
class withdrawalSavings:
    def __init__(self):
        self.window = Toplevel()
        self.window.title("Savings Account")
        self.window["bg"] = "dark olive green"

        label = Label(self.window, text = "How much would you like to withdraw?", font = PROMPT_FONT, fg = "gray99")
        label.pack()
        label["bg"] = "dark olive green"

        self.withdrawalAmount = StringVar()
        entrySavings = Entry(self.window, textvariable = self.withdrawalAmount)
        entrySavings.pack()

        submitBtn = Button(self.window, text = "Submit", font = BUTTON_FONT, bg = "olive drab", fg = "gray99", command = self.withdrawalSavings)
        submitBtn.pack()

    def withdrawalSavings(self):
        global SAVINGS_BALANCE
        global CHECKING_BALANCE
        if(float(self.withdrawalAmount.get()) > 0):
            SAVINGS_BALANCE -= float(self.withdrawalAmount.get())
            TRANSACTION_HISTORY.append("Account: Savings || Type:  Withdrawal ||" + " Amount: " + self.withdrawalAmount.get() +
                                       " || Checking Balance: " + str(format(CHECKING_BALANCE, ".2f")) + " Savings Balance: " + str(format(SAVINGS_BALANCE, ".2f")))
            tkinter.messagebox.showinfo("Withdrawn", "Withdrawal successful! " + "savings $" + str(format(SAVINGS_BALANCE, ".2f")) +
                    " checking $" + str(format(CHECKING_BALANCE, ".2f")))
        self.window.destroy()
        

#################################
#create checking account windows#
#################################
...
class balanceChecking:
    ...
    def viewChecking(self,value):
        if(value=="yes"):
                global CHECKING_BALANCE
                tkinter.messagebox.showinfo("Checking", "Your balance in " + "checking is $" + str(format(CHECKING_BALANCE, ".2f")))
        self.window.destroy()
...
#create window to withdraw from checking account
class withdrawalChecking:
    def __init__(self):
        self.window = Toplevel()
        self.window.title("Checking Account")
        self.window["bg"] = "dark olive green"
        ...
        submitBtn = Button(self.window, text = "Submit", font = BUTTON_FONT, bg = "olive drab", fg = "gray99", command = self.withdrawalChecking)
        submitBtn.pack()

    def withdrawalChecking(self):
        global SAVINGS_BALANCE
        global CHECKING_BALANCE
        if(float(self.withdrawalAmount.get()) > 0):
            CHECKING_BALANCE -= float(self.withdrawalAmount.get())
            TRANSACTION_HISTORY.append("Account: Checking || Type:  Withdrawal ||" + " Amount: " + self.withdrawalAmount.get() +
                                       " || Checking Balance: " + str(format(CHECKING_BALANCE, ".2f")) + " Savings Balance: " + str(format(SAVINGS_BALANCE, ".2f")))
            tkinter.messagebox.showinfo("withdrawn", "withdrawal successful! " + "savings $" + str(format(SAVINGS_BALANCE, ".2f")) +
                    " checking $" + str(format(CHECKING_BALANCE, ".2f")))
        self.window.destroy()
